/// <reference path="Scripts/jquery.d.ts" />
import V = require("./VCL");
import VXC = require("./VXComponent");
import VXD = require("./VXDataset");
import VXCB = require("./VXChartBase");
export declare class TChartBar extends VXCB.TGridChartBase {
    /** Custom Format value on top */
    LabelOnTopFormat: (data: any) => string;
    ImageOnTopFormat: (data: any) => string;
    /** Custom onClick */
    onClicked: (value: V.TBarValue, series: number, idx: number) => void;
    /** The Bars */
    private bar;
    selectedItem(series: number, idx: number, fireEvents: boolean): void;
    private _showValueOnTop;
    ShowValueOnTop: boolean;
    private _showImageOnTop;
    ShowImageOnTop: boolean;
    private _maximumbarwidth;
    MaximumBarWidth: number;
    private _minY;
    YMin: number;
    private _maxY;
    YMax: number;
    private _selectcolorOpacity;
    /**
    * set the opacity of the selected bar  - values (0-1)
    */
    SelectedBarOpacity: number;
    private _unselectcolorOpacity;
    /**
    * set the opacity of the unselected bars - values (0-1)
    */
    UnselectedBarOpacity: number;
    private _series1color;
    Series1Color: string;
    private _series2color;
    Series2Color: string;
    private _series3color;
    Series3Color: string;
    private _series4color;
    Series4Color: string;
    private _series5color;
    Series5Color: string;
    private _series6color;
    Series6Color: string;
    private _series7color;
    Series7Color: string;
    private _series8color;
    Series8Color: string;
    private _series9color;
    Series9Color: string;
    private _series10color;
    Series10Color: string;
    private _series11color;
    Series11Color: string;
    private _series12color;
    Series12Color: string;
    private _series13color;
    Series13Color: string;
    private _series14color;
    Series14Color: string;
    private _series15color;
    Series15Color: string;
    private _series1name;
    Series1Name: string;
    private _series2name;
    Series2Name: string;
    private _series3name;
    Series3Name: string;
    private _series4name;
    Series4Name: string;
    private _series5name;
    Series5Name: string;
    private _series6name;
    Series6Name: string;
    private _series7name;
    Series7Name: string;
    private _series8name;
    Series8Name: string;
    private _series9name;
    Series9Name: string;
    private _series10name;
    Series10Name: string;
    private _series11name;
    Series11Name: string;
    private _series12name;
    Series12Name: string;
    private _series13name;
    Series13Name: string;
    private _series14name;
    Series14Name: string;
    private _series15name;
    Series15Name: string;
    private _showhoverlegend;
    ShowHoverLegend: boolean;
    private _barleftopradius;
    BarLeftTopRadius: number;
    private _barlefbottomradius;
    BarLeftBottomRadius: number;
    private _barRightopradius;
    BarRightTopRadius: number;
    private _barrightbottomradius;
    BarRightBottomRadius: number;
    private _stack;
    Stacked: boolean;
    private _defSelectionLast;
    SetLastSelected: boolean;
    values: VXCB.TChartValuesCollection<VXCB.TBarValue>;
    createValue(label: string, value1?: number, value2?: number, value3?: number, value4?: number, value5?: number, value6?: number, value7?: number, value8?: number, value9?: number, value10?: number, value11?: number, value12?: number, value13?: number, value14?: number, value15?: number): VXCB.TBarValue;
    getData(): any[];
    draw(reCreate: boolean): void;
    createBar(): Bar;
    create(): void;
}
export declare class TDBChartBar extends TChartBar {
    private _value1field;
    /**
    * Specifies the field from which the edit control displays data.
    */
    ValueField1: string;
    private _value2field;
    /**
    * Specifies the field from which the edit control displays data.
    */
    ValueField2: string;
    private _value3field;
    /**
    * Specifies the field from which the edit control displays data.
    */
    ValueField3: string;
    private _value4field;
    /**
    * Specifies the field from which the edit control displays data.
    */
    ValueField4: string;
    private _value5field;
    /**
    * Specifies the field from which the edit control displays data.
    */
    ValueField5: string;
    private _value6field;
    /**
    * Specifies the field from which the edit control displays data.
    */
    ValueField6: string;
    private _value7field;
    /**
    * Specifies the field from which the edit control displays data.
    */
    ValueField7: string;
    private _value8field;
    /**
    * Specifies the field from which the edit control displays data.
    */
    ValueField8: string;
    private _value9field;
    /**
    * Specifies the field from which the edit control displays data.
    */
    ValueField9: string;
    private _value10field;
    /**
    * Specifies the field from which the edit control displays data.
    */
    ValueField10: string;
    private _value11field;
    /**
    * Specifies the field from which the edit control displays data.
    */
    ValueField11: string;
    private _value12field;
    /**
    * Specifies the field from which the edit control displays data.
    */
    ValueField12: string;
    private _value13field;
    /**
    * Specifies the field from which the edit control displays data.
    */
    ValueField13: string;
    private _value14field;
    /**
    * Specifies the field from which the edit control displays data.
    */
    ValueField14: string;
    private _value15field;
    /**
    * Specifies the field from which the edit control displays data.
    */
    ValueField15: string;
    private _labelfield;
    /**
    * Specifies the field from which the edit control displays data.
    */
    LabelField: string;
    private _dataset;
    /**
      * Specifies the dataset that contains the field it represents.
    */
    Dataset: VXD.TDataset;
    create(): void;
    getData(): any[];
}
export declare class Bar extends VXCB.Grid {
    bars: any;
    constructor(options: any, owner: any);
    init(): void;
    calc(): void;
    calcBars(): any;
    draw(): void;
    drawXAxis(): any;
    barNodes: any[];
    drawSeries(): any;
    colorFor(row: any, sidx: any, type: any): any;
    hitTest(x: any, y: any): number;
    hitSeries(x: any, y: any): number;
    clickItem(idx: number, series: number, fireEvents?: boolean): void;
    onGridClick(x: any, y: any, evt: any): void;
    private oldTipId;
    onHoverMove(x: any, y: any, evt: any): void;
    hoverItem(idx: any, series: any): any[];
    onHoverOut(): any[];
    displayHoverForRow(index: any, series: any): any[];
    roundedRect(x: any, y: any, w: any, h: any, r?: number[]): any[];
    drawBar(row: any, idx: any, xPos: any, yPos: any, width: any, height: any, barColor: any, oldheight: any, oldy: any, series: any, cornes?: number[]): any;
    labelOnTopFormat(data: any, humanFriendly: boolean): string;
}
export declare class TChartBullet extends VXC.TComponent {
    constructor(aOwner: VXC.TComponent, renderTo?: string);
    private _title;
    Title: string;
    private _titlecolor;
    TitleColor: string;
    private _value;
    Value: number;
    private _maximum;
    Maximum: number;
    private _showsubtitle;
    ShowSubTitle: boolean;
    private _showvalue;
    ShowValue: boolean;
    private _valuefontsize;
    ValueFontSize: number;
    draw(reCreate: boolean): void;
    create(): void;
}
